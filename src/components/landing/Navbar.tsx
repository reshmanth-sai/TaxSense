...
import { Sun, Moon, ArrowRight, ChevronDown, Menu, X, ShieldCheck, Sparkles, FileText, ExternalLink } from 'lucide-react';
...
export const Navbar: React.FC<NavbarProps> = ({ onStart, activeSection }) => {
  ...
  const primaryLinks = [
    { id: 'comparison', label: 'Compare Regimes' },
    { id: 'interactive-showcase', label: 'Tax Planner' },
  ...
  return (
    <div className="fixed top-11 left-1/2 -translate-x-1/2 z-40 w-full max-w-[1200px] px-4 sm:px-6 pointer-events-none select-none">
        <motion.header
          ...
          className={`pointer-events-auto w-full flex items-center justify-between rounded-[22px] transition-all duration-300 border ${
            isScrolled
              ? 'h-[60px] sm:h-[64px] px-4 sm:px-5 bg-white/85 border-white/90 dark:bg-[#0B1220]/85 dark:border-white/[0.1] backdrop-blur-[28px] shadow-[0_12px_44px_rgba(0,0,0,0.08)] dark:shadow-[0_16px_50px_rgba(0,0,0,0.5)]'
              : 'h-[68px] sm:h-[72px] px-5 sm:px-6 bg-white/60 border-white/70 dark:bg-[#0B1220]/65 dark:border-white/[0.07] backdrop-blur-[24px] shadow-[0_8px_32px_rgba(0,0,0,0.04)]'
          }`}
        >
          {/* LEFT SECTION: BRANDING & LIVING AI PULSE */}
          ...
          <nav className="hidden md:flex items-center gap-5 lg:gap-6 text-[13px] font-sans font-medium text-slate-600 dark:text-slate-300">
            ...
            {/* RICH RESOURCES DROPDOWN TRIGGER */}
                    ...
                    {moreLinks.map((item) => (
                      item.external ? (
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="px-3 py-2 rounded-xl hover:bg-slate-100 dark:hover:bg-white/5 text-slate-650 dark:text-slate-300 hover:text-[#0B1730] dark:hover:text-white transition-colors flex items-center justify-between cursor-pointer"
                        >
                          <span>{item.label}</span>
                          <ExternalLink className="w-3 h-3 text-slate-400 font-mono" />
                        </a>
                      ) : (
                        <button
                          key={item.id}
          ...
          {/* RIGHT SECTION: CLEAN THEME TOGGLE & OUTCOME CTA */}
